...
export default function GasStationCard({
  ...
  const fuelTypeMap: Record<
    string,
    { key: keyof typeof price; label: string }
  > = {
    ...
    <Link href={`/gas/${id}?zipcode=${searchedZipCode}`}>
      <div className="bg-[#E8D9B5] rounded-lg p-4 sm:p-6 mb-4 shadow-md cursor-pointer hover:shadow-lg transition-shadow">
        <div className="flex flex-col">
          <div className="mb-3">
            <h3 className="text-lg sm:text-xl font-semibold text-[#3D405B]">
              {name}
            </h3>
            <div className="mt-1">
              ...
              <p className="text-sm sm:text-base text-[#3D405B]/80">
                {address.cityStateZip.replace(/(\d{5})-\d+/g, "$1") ||
                  `${city}, ${formatZipCode(zipCode)}`}
              </p>
            </div>
          </div>
          ...
          <div className="mt-2">
            <h4 className="text-sm font-semibold text-[#3D405B] mb-2">
              Fuel Prices:
            </h4>
                ...
                return (
                  <div key={fuelType} className="p-2 rounded bg-[#D9C9A3]">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="font-bold text-[#008737]">
